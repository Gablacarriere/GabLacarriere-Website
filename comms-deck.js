...
function clean(){generation++;user=null;threads=[];selected=null;replies=[];$('threadList').replaceChildren();$('threadContent').replaceChildren();$('replyList').replaceChildren();$('threadForm').reset();$('replyForm').reset();$('crewWorkspace').hidden=true;$('crewLogin').hidden=false;$('crewMemberList').replaceChildren();roster=[];}
...
async function list(reset=true){
 const run=++generation;if(reset){offset=0;threads=[];}const wanted=channel;
 ...
 $('threadList').innerHTML=threads.map(t=>`<button class="threadRow" data-thread="${t.id}"><strong>${esc(t.title)}</strong><small>${who(t)} · ${esc(date(t.created_at))}${t.concept_id?' · '+esc(label(t.concept_id)):''}</small></button>`).join('');
 $('crewStatus').textContent=threads.length?'Up to date. Use Refresh conversations to check for new messages.':'No conversations here yet. '+(wanted==='announcements'?'Coach updates will appear here.':'You can start the first one.');
}
...
$('closeThread').onclick=()=>choose(channel);$('refreshCrew').onclick=()=>{if(coach)crewAccess();return selected?openThread(selected.id):list();};$('moreThreads').onclick=()=>list(false);$('moreReplies').onclick=()=>selected&&openThread(selected.id,true);
...
(async()=>{try{
 ...
 $('threadConcept').innerHTML=concepts.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');$('crewWorkspace').hidden=false;
