(() => {
  ...
  let db=null,user=null,role='mentee',rows=new Map(),refreshing=false,refreshQueued=false;
  ...
  function uniqueSkills(nodeId){
    ...
    const seen=new Set();
    return links.filter(x=>x?.skill&&!seen.has(x.skill)&&seen.add(x.skill));
  }
  ...
  function enhance(){
    document.querySelectorAll('.roadmapStop[data-roadmap-id]').forEach(card=>{
      ...
      if(role!=='coach'){
        ...
        if(!skills.length){const span=document.createElement('span');span.className='roadmapNoPractice';span.textContent='Zoukable drill link not mapped yet';actions.appendChild(span);}
      }
