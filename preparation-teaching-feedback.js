...
let scheduled=false,historyLoaded=false,historyState=null,historySession='';
...
function specificRows(){
  return [...root.querySelectorAll('[data-activity][data-field="title"]')].map(title=>{
    const id=title.dataset.activity,row=title.closest('.toolRow');
    const instructions=root.querySelector(`[data-activity="${id}"][data-field="instructions"]`);
    return{id,title:title.value||'',row,instructions};
  }).filter(x=>x.row&&x.instructions&&/^specific warm-up/i.test(x.title));
}
function currentMode(activity){
  const text=activity.instructions.value||'';
  if(/— Generated variation: Easier —/.test(text))return'easy';
  if(/— Generated variation: Challenge —/.test(text))return'challenge';
  return'standard';
}
function plannedAdaptations(activity){
  const value=field('adaptations')?.value||'';
  const safe=activity.title.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  const re=new RegExp(`— Adaptation branch · ${safe} · ([^—\\n]+) —`,'g');
  const out=[];let m;
...
function parseFeedbackBlocks(text){
  ...
  while((m=re.exec(String(text||'')))){
    ...
    out.push({
      ...
      version:get('Version used:')||'standard',
      outcome:get('Outcome:')||'mixed',
      adaptations:get('Adaptations used:').split(' | ').filter(x=>x&&x!=='None recorded'),
...
function feedbackBlock(activity,{version,outcome,adaptations,observation}){
  const next=recommendation(version,outcome,adaptations);
  const obs=String(observation||'').replace(/\s*\n\s*/g,' / ').trim()||'No additional note.';
...
function panelValues(panel){
  return{
    version:panel.querySelector('[data-feedback-version]')?.value||'standard',
    outcome:panel.querySelector('[data-feedback-outcome]')?.value||'mixed',
    adaptations:[...panel.querySelectorAll('[data-feedback-adaptation]:checked')].map(x=>x.value),
    observation:panel.querySelector('[data-feedback-observation]')?.value||''
  };
}
function updatePreview(panel){
  const v=panelValues(panel),el=panel.querySelector('.feedbackRecommendation');
  if(el)el.textContent=recommendation(v.version,v.outcome,v.adaptations);
}
function renderActivity(activity){
  let panel=activity.row.querySelector(':scope > .prepFeedback');
  if(panel?.dataset.dirty==='1')return;
  const saved=currentFeedback(activity.id),branches=plannedAdaptations(activity);
  const mode=saved?.version||currentMode(activity),outcome=saved?.outcome||'worked';
  const sig=[activity.title,mode,outcome,branches.join('|'),saved?.observation||'',saved?.next||''].join('||');
  if(!panel){
    panel=document.createElement('section');panel.className='prepFeedback';
    const adapt=activity.row.querySelector(':scope > .adaptEngine'),progress=activity.row.querySelector(':scope > .progressionLadder');
    if(adapt)adapt.after(panel);else if(progress)progress.after(panel);else activity.row.querySelector('.toolRowActions')?.before(panel);
  }
  if(panel.dataset.signature===sig)return;
  panel.dataset.signature=sig;panel.dataset.dirty='0';
...
function renderRows(){
  specificRows().forEach(renderActivity);
  root.querySelectorAll('.toolRow > .prepFeedback').forEach(panel=>{
    const row=panel.closest('.toolRow'),title=row?.querySelector('[data-activity][data-field="title"]')?.value||'';
    if(!/^specific warm-up/i.test(title))panel.remove();
  });
}
function saveFeedback(id){
  const activity=specificRows().find(x=>x.id===id),panel=activity?.row.querySelector(':scope > .prepFeedback'),area=reflectionEl();
  if(!activity||!panel||!area)return;
  const values=panelValues(panel),base=stripFeedback(area.value,id),block=feedbackBlock(activity,values);
  area.value=`${base}${base?'\n\n':''}${block}`;area.dispatchEvent(new Event('input',{bubbles:true}));panel.dataset.dirty='0';
  const out=document.getElementById('toolStatus');
  if(out)out.textContent='Preparation teaching evidence saved in this class reflection. It can inform the next session in this curriculum.';
  schedule();
}
function clearFeedback(id){
  const area=reflectionEl();if(!area)return;
  const next=stripFeedback(area.value,id);if(next===area.value)return;
  area.value=next;area.dispatchEvent(new Event('input',{bubbles:true}));
  const panel=specificRows().find(x=>x.id===id)?.row.querySelector(':scope > .prepFeedback');
  if(panel)panel.dataset.dirty='0';
  schedule();
}

function stripHistoryBlock(value){
  return String(value||'').replace(/\n*— Previous-class preparation evidence —[\s\S]*?— End previous-class preparation evidence —\n*/g,'\n').trim();
}
function historyBlock(prev,items){
  const useful=items.filter(x=>x.outcome!=='not_used').slice(0,4);
...
function carryHistory(){
  if(!historyState?.previous)return;
  const block=historyBlock(historyState.previous,historyState.feedback),area=readinessEl();
  if(!block||!area)return;
  const base=stripHistoryBlock(area.value),next=`${base}${base?'\n\n':''}${block}`;
  if(next!==area.value){area.value=next;area.dispatchEvent(new Event('input',{bubbles:true}));}
}
function removeCarry(){
  const area=readinessEl();if(!area)return;
  const next=stripHistoryBlock(area.value);
  if(next!==area.value){area.value=next;area.dispatchEvent(new Event('input',{bubbles:true}));}
}
function toggleCarry(){
  const area=readinessEl();if(!area)return;
  if((area.value||'').includes(HISTORY_START)){
    removeCarry();
    const out=document.getElementById('toolStatus');if(out)out.textContent='Previous-class preparation evidence removed from this plan.';
  }else{
    carryHistory();
    const out=document.getElementById('toolStatus');if(out)out.textContent='Previous-class preparation evidence restored to readiness notes.';
  }
  renderHistory();
}
function renderHistory(){
  const panel=root.querySelector('.prepRecoveryPanel');
  if(!panel||!historyLoaded)return;
  let box=document.getElementById('prepTeachingHistory');
  if(!box){
    box=document.createElement('section');box.id='prepTeachingHistory';box.className='prepHistory';
    const anchor=document.getElementById('movementDemandMap')||panel.querySelector('.prepGraph');
    if(anchor)anchor.before(box);else panel.appendChild(box);
  }
  if(!historyState?.previous){
    box.innerHTML='<p class="kicker">TEACHING FEEDBACK LOOP</p><h4>No previous preparation evidence yet</h4><p>After you save evidence in a class, the next session in the same curriculum will inherit the useful preparation notes here.</p>';
    return;
  }
  const useful=historyState.feedback.filter(x=>x.outcome!=='not_used');
  const carried=(readinessEl()?.value||'').includes(HISTORY_START);
  box.innerHTML=`<p class="kicker">EVIDENCE FROM LAST CLASS</p><h4>${esc(historyState.previous.title||'Previous session')}</h4><p>This evidence can feed the current session’s readiness notes so the demand, progression and adaptation tools use what happened last time.</p>${historyState.feedback.map(x=>`<div class="prepHistoryItem"><strong>${esc(x.activity||'Preparation activity')}</strong><span>${esc(outcomeLabel[x.outcome]||x.outcome)} · ${esc(versionLabel[x.version]||x.version)}</span>${x.observation&&x.observation!=='No additional note.'?`<p>${esc(x.observation)}</p>`:''}<small>${esc(x.next||'')}</small></div>`).join('')}${useful.length?`<button type="button" data-toggle-prep-history>${carried?'Remove carried evidence from this plan':'Restore evidence to readiness notes'}</button>`:'<p class="toolHint">Nothing actionable was carried because the recorded preparation was not used.</p>'}`;
}
async function loadHistory(){
  if(historyLoaded)return;
  const sid=sessionId();
  if(!sid||!window.TeacherCloud?.read)return;
  try{
    if(window.TeacherPlannerFlush)await window.TeacherPlannerFlush();
    const result=await window.TeacherCloud.read(),doc=result?.document,current=doc?.sessions?.find(x=>x.id===sid);
    if(!current){historyLoaded=true;historyState=null;renderHistory();return;}
    const course=doc.courses?.find(x=>x.id===current.courseId);
    if(!course){historyLoaded=true;historyState=null;renderHistory();return;}
    const index=course.sessionIds.indexOf(current.id);let previous=null,feedback=[];
    for(let i=index-1;i>=0;i--){
      const candidate=doc.sessions.find(x=>x.id===course.sessionIds[i]);if(!candidate)continue;
      const blocks=parseFeedbackBlocks(candidate.reflection||'');
      if(blocks.length){previous=candidate;feedback=blocks;break;}
    }
    historyState=previous?{previous,feedback}:null;historyLoaded=true;
    if(historyState)carryHistory();
    renderHistory();
  }catch(error){
    historyLoaded=true;historyState=null;renderHistory();
    console.warn('Preparation teaching feedback history unavailable',error);
  }
}
function schedule(){
  ...
  requestAnimationFrame(()=>{
    scheduled=false;
    const sid=sessionId();
    if(sid!==historySession){
      historySession=sid;historyLoaded=false;historyState=null;
      document.getElementById('prepTeachingHistory')?.remove();
    }
    installStyle();renderRows();renderHistory();
    if(!historyLoaded&&reflectionEl())loadHistory();
  });
}

root.addEventListener('input',event=>{
  const panel=event.target.closest('.prepFeedback');
  if(panel){panel.dataset.dirty='1';updatePreview(panel);return;}
...
root.addEventListener('change',event=>{
  const panel=event.target.closest('.prepFeedback');
  if(panel){panel.dataset.dirty='1';updatePreview(panel);return;}
  if(event.target.dataset.concept||event.target.dataset.skill||event.target.matches('select[data-field]'))schedule();
});
root.addEventListener('click',event=>{
  const save=event.target.closest('[data-save-prep-feedback]');
  if(save){event.preventDefault();event.stopPropagation();saveFeedback(save.dataset.savePrepFeedback);return;}
  const clear=event.target.closest('[data-clear-prep-feedback]');
  if(clear){event.preventDefault();event.stopPropagation();clearFeedback(clear.dataset.clearPrepFeedback);return;}
  if(event.target.closest('[data-toggle-prep-history]')){event.preventDefault();event.stopPropagation();toggleCarry();}
});
