(() => {
  ...
  const curriculum=window.ATLAS_CURRICULUM_V2;
  const rawNodes=window.ZOUK_ATLAS.flatMap(g=>g.topics.map((name,i)=>({id:g.id+'-'+i,sourceName:name})));
  const nodes=rawNodes.map(n=>({id:n.id,name:curriculum?.displayName(n.id)||n.sourceName,family:curriculum?.familyFor(n.id)?.name||'Curriculum'}));
  ...
  function conceptFieldset(){
    if(!curriculum)return window.ZOUK_ATLAS.map(g=>'<details><summary>'+esc(g.name)+'</summary>'+g.topics.map((name,i)=>'<label class="conceptChoice"><span>'+esc(name)+'</span><select data-concept="'+g.id+'-'+i+'"><option value="">Not covered</option><option>Introduced</option><option>Practicing</option><option>Integrating</option></select></label>').join('')+'</details>').join('');
    return curriculum.families.map(f=>{
      const familyNodes=curriculum.nodesByFamily(f.id);
      return '<details><summary>'+esc(f.name)+'</summary><p class="guideLabel">'+esc(f.intro)+'</p>'+familyNodes.map(n=>'<label class="conceptChoice"><span>'+esc(curriculum.displayName(n.id))+' <small>· '+['','Core','Foundation','Integration','Application','Exploration'][curriculum.meta[n.id].tier]+'</small></span><select data-concept="'+esc(n.id)+'"><option value="">Not covered</option><option>Introduced</option><option>Practicing</option><option>Integrating</option></select></label>').join('')+'</details>';
    }).join('');
  }
  function form(s){
    const date=new Date();date.setMinutes(date.getMinutes()-date.getTimezoneOffset());
    return '<form id="atlasLessonForm" class="box atlasLessonForm"><h2>Record a lesson for '+esc(s.student.display_name)+'</h2><p>Everything you write here is visible to this student. Select only concepts you actually covered. The most recent lesson date sets each concept’s current status.</p><label>Lesson date<input name="lesson_date" type="date" required max="'+date.toISOString().slice(0,10)+'" value="'+date.toISOString().slice(0,10)+'"></label><label>What did you explore?<textarea name="summary" required maxlength="3000" rows="3" placeholder="A short summary of the lesson"></textarea></label><fieldset><legend>Concepts covered</legend><p>Leave untouched concepts as “Not covered”. The domains describe curriculum logic; they are not levels of student ability.</p>'+conceptFieldset()+'</fieldset><label>Practice focus (optional)<textarea name="practice" maxlength="2000" rows="3" placeholder="What should the student explore before the next lesson?"></textarea></label><p>Discovery is not a claim of mastery. Saving this lesson updates the student’s map immediately.</p><button class="primaryButton" type="submit">Save lesson & update map</button><p id="lessonSaveStatus" role="status"></p></form>';
  }
  function markup(view,s){
    ...
    return '<h1>'+esc(s.student.display_name)+'’s sessions</h1><p>Coach-reviewed lessons and discoveries.</p>'+(coach?form(s):'')+((coach?s.lessons:list).length?s.lessons.filter(l=>coach||!l.voided).map(l=>card(l)+(coach?'<p><button class="secondaryButton" data-void-lesson="'+esc(l.id)+'" data-voided="'+String(!l.voided)+'">'+(l.voided?'Restore lesson':'Remove lesson from map')+'</button></p>':'')).join(''):'<div class="box"><h2>No recorded lessons yet.</h2><p>Your map is waiting for its first recorded lesson—not for you to learn everything from scratch. Gab or Steph will add what you explore together.</p></div>')+'<p id="lessonActionStatus" role="status"></p>';
  }
