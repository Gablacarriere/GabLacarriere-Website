(() => {
'use strict';
const cfg=window.GAB_PORTAL,root=document.getElementById('teachingWorkbench'),bar=document.getElementById('accountBar');
const esc=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
let client,user=null,initializing=true;
function gate(message='') {root.innerHTML=`<section class="toolPanel accountGate"><p class="kicker">Your teaching studio</p><h2>Sign in to your website account.</h2><p>Your Zouk and Lambada plans stay together here, ready when you return on any device.</p><form id="plannerLogin"><label>Email<input type="email" name="email" autocomplete="username" required></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button class="toolPrimary" type="submit">Sign in &amp; open my plans</button><p id="plannerLoginMessage" role="status">${esc(message)}</p></form><p><a href="/mentorship-hub/">Password help / member account</a> · <a href="/for-teachers/#teacher-contact">Ask Gab for teacher access</a></p><p class="toolHint">Use the same login as the rest of the website. Signing in here does not change your mentorship membership.</p></section>`;}
...
(() => {
  ...
  if (document.querySelector('script[data-teaching-session-bridge]')) return;
