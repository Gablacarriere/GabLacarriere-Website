# Zouk Atlas Curriculum v2

## Purpose

Curriculum v2 separates three things that the first Atlas mixed together:

1. **Stable record IDs** — the IDs stored in student lesson history.
2. **Student-facing curriculum structure** — where a concept belongs pedagogically today.
3. **Conceptual relationships** — which ideas support, constrain, or explain other ideas.

Existing IDs are preserved. The curriculum can therefore evolve without rewriting historical student records.
...
- frame, light continuous connection, changing contact points and partner position;
- follower responsiveness rather than anticipatory pattern execution;
...
The map uses two independent dimensions:

- **Radius = curriculum depth**, not student ability.
- **Angular neighborhood = functional similarity.**

Relationships are support relations rather than hard prerequisites. A learner can encounter an advanced idea early, but the map makes visible which earlier mechanisms help explain or stabilize it.

### Depth bands

1. **Core** — high-reuse ideas that organize many later actions.
2. **Foundations** — mechanisms that build on the core and recur across families.
3. **Integration** — combining foundations into partner and movement tasks.
...
Breath, posture, tone, full weight transfer, step function, grounding, joint availability and axis.

**Logic:** the Granola notes repeatedly treat complete transfer and body organization as prerequisites for reliable partner information, rotation, counterbalance and head movement.

### 2. Frame & Connection

Frame/contact organization, connection principles, permeability, active contact point, embrace continuum, follower responsiveness, tone modulation and connection preservation.

**Logic:** frame is the organization that makes contact usable; connection is the information channel. These should be near each other but not collapsed into one term.

### 3. Rhythm & Timing

Pulse/counting, Traditional references, timing families, phrasing, availability-and-timing, timing-through-connection and Lambada question–answer timing.

**Logic:** timing has its own dependency chain and should not be housed inside Connection merely because timing can be communicated through connection.

### 4. Space & Orientation

Chest direction, relative position, orientation, positioning and blocking/space creation.
...
- `connection-2` displays as **Full Weight Transfer & Commitment**.
- `steps-1` displays as **Footwork Mechanics & Grounding**.
- `bridge-2` displays as **Axis & Directional Freedom**.
- `architecture-0` displays as **Chest Direction & Relative Position** and is moved inward into Space & Orientation.
- `connection-3` displays as **Traditional Timing References** inside Rhythm & Timing.
...
- Whether every Saltinho/Sortinho label refers to exactly the same execution across Zouk and Lambada.
