...
const context={URLSearchParams,window:{addEventListener(e,fn){this[e]=fn}},document:{createElement:()=>element('newlink'),getElementById:id=>id==='atlasLessonForm'?null:element(id),querySelectorAll:()=>[]},location:{hash:'#map',search:''},requestAnimationFrame:fn=>fn(),setTimeout:fn=>fn()};vm.createContext(context);vm.runInContext(fs.readFileSync('mentorship-crew.js','utf8'),context);vm.runInContext(fs.readFileSync('zouk-map-data.js','utf8'),context);vm.runInContext(fs.readFileSync('atlas-universe.js','utf8'),context);vm.runInContext(fs.readFileSync('atlas-live.js','utf8'),context);vm.runInContext(fs.readFileSync('zouk-map.js','utf8'),context);
context.window['atlas-state']({detail:{mode:'guest'}});assert(element('content').innerHTML.includes('0 / 56'));assert(element('galaxy').innerHTML.includes('fogLayer'));assert(element('galaxy').innerHTML.includes('MENTORSHIP MOTHERSHIP'));assert(!element('galaxy').innerHTML.includes('data-node='));assert(element('detail').innerHTML.includes('A universe to discover'));
