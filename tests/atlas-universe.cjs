...
assert.equal(total,80);assert.equal(curriculum.families.length,8);assert.equal(curriculum.familyFor('connection-3').id,'rhythm');assert.equal(curriculum.familyFor('architecture-0').id,'space');assert.equal(curriculum.displayName('connection-2'),'Full Weight Transfer & Commitment');assert.equal(curriculum.displayName('architecture-0'),'Chest Direction & Relative Position');
assert(curriculum.related('connection-6').some(r=>r.id==='connection-0'));assert(curriculum.related('pathways-0').some(r=>r.id==='spirals-0'));
