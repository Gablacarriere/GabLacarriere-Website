...
const context={URLSearchParams,window:{addEventListener(e,fn){this[e]=fn}},document:{createElement:()=>element('newlink'),getElementById:id=>id==='atlasLessonForm'?null:element(id),querySelectorAll:()=>[]},location:{hash:'#map',search:''},requestAnimationFrame:fn=>fn(),setTimeout:fn=>fn()};vm.createContext(context);vm.runInContext(fs.readFileSync('mentorship-crew.js','utf8'),context);vm.runInContext(fs.readFileSync('zouk-map-data.js','utf8'),context);vm.runInContext(fs.readFileSync('atlas-foundation-extension.js','utf8'),context);vm.runInContext(fs.readFileSync('atlas-universe.js','utf8'),context);vm.runInContext(fs.readFileSync('atlas-live.js','utf8'),context);vm.runInContext(fs.readFileSync('zouk-map.js','utf8'),context);
context.window['atlas-state']({detail:{mode:'guest'}});assert(element('content').innerHTML.includes('0 / 57'));assert(element('galaxy').innerHTML.includes('fogLayer'));assert(element('galaxy').innerHTML.includes('MENTORSHIP MOTHERSHIP'));assert(!element('galaxy').innerHTML.includes('data-node='));assert(element('detail').innerHTML.includes('A universe to discover'));
...
element('previewToggle').onclick();assert.equal((element('galaxy').innerHTML.match(/data-node=/g)||[]).length,57);assert(!element('galaxy').innerHTML.includes('fogLayer'));assert(element('content').innerHTML.includes('0 / 57'));
context.location.hash='#sessions';context.window.hashchange();element('content').listeners.click({target:{closest:s=>s==='#approve'?{}:null}});
context.location.hash='#map';context.window.hashchange();element('previewToggle').onclick();assert.equal((element('galaxy').innerHTML.match(/data-node=/g)||[]).length,3);assert(element('content').innerHTML.includes('3 / 57'));
...
element('reset').listeners.click();context.location.hash='#map';context.window.hashchange();assert(element('content').innerHTML.includes('0 / 57'));
console.log('PASS: starts at ship with zero discoveries; fog; 57-node curriculum preview; preview leaves progress unchanged; zoom; recenter; lesson reveals three nodes; practice; reset.');
...
assert(element('content').innerHTML.includes('1 / 57'));
assert.equal((element('galaxy').innerHTML.match(/data-node=/g)||[]).length,1);
element('previewToggle').onclick();
assert.equal((element('galaxy').innerHTML.match(/data-node=/g)||[]).length,1);
element('reset').listeners.click();assert(element('content').innerHTML.includes('1 / 57'));
...
assert.equal((element('galaxy').innerHTML.match(/data-node=/g)||[]).length,57);
...
assert.equal(Object.keys(context.window.ZOUK_DETAILS).length,57);
for(const d of Object.values(context.window.ZOUK_DETAILS))assert(d.understand.length>70&&d.notice.length>25);
assert.equal(context.window.ZOUK_ATLAS.find(g=>g.id==='connection').topics[6],'Frame / Contact Organization');
assert(u.related('connection-6').some(r=>r.id==='organization-1'&&r.strength===3));
assert(u.related('connection-6').some(r=>r.id==='connection-0'&&r.strength===3));
...
console.log('PASS: all 57 study guides, Frame bridge relationships, bounded non-overlapping geometry, landmark navigation does not award progress.');
...
assert(search('frame').includes('connection-6'));
...
assert.equal(element('discoverySelect').value,'organization-0');
assert.equal(search(''),'');
console.log('PASS: accent-insensitive search, Frame search, constellation search, undiscovered concepts excluded, selected result zooms and syncs navigation, progress unchanged, unavailable selection rejected.');
